"""
F4: 7-Subcomponent Rotated Elliptic / 7 子组件旋转椭球函数.

CEC 2013 大规模全局优化基准测试函数 F4。
CEC 2013 Large-Scale Global Optimization Benchmark Function F4.

函数特性 / Function Characteristics:
    - 多子组件分解 / Multi-subcomponent decomposition
    - 7 个子组件 / 7 sub-components
    - 旋转和位移 / Rotation and shift
    - Elliptic 函数 / Elliptic function
"""

from typing import Dict, List, Tuple, Union

import numpy as np
import numpy.typing as npt

from .benchmarks import Benchmarks


class F4(Benchmarks):
    """
    F4: 7-Subcomponent Rotated Elliptic / 7 子组件旋转椭球函数.

    CEC 2013 基准测试函数 F4，使用 Elliptic 函数作为子组件。
    CEC 2013 benchmark function F4 using Elliptic function as sub-components.

    Attributes:
        ID: 函数标识符 / Function identifier (4)
        s_size: 子问题数量 / Number of sub-problems (7)
        minX: 搜索下界 / Lower search bound (-100.0)
        maxX: 搜索上界 / Upper search bound (100.0)
    """

    def __init__(self) -> None:
        """初始化 F4 函数 / Initialize F4 function."""
        super().__init__()
        self.ID = 4
        self.s_size = 7
        self.Ovector = self.readOvector()
        self.Pvector = self.readPermVector()
        self.r25 = self.readR(25)
        self.r50 = self.readR(50)
        self.r100 = self.readR(100)
        self.s = self.readS(self.s_size)
        self.w = self.readW(self.s_size)
        self.minX = -100.0
        self.maxX = 100.0

        # 预计算子问题配置 / Pre-compute sub-problem configuration
        self.sub_problems: List[Tuple[npt.NDArray, npt.NDArray, float]] = []
        c = 0
        rot_map = {25: self.r25, 50: self.r50, 100: self.r100}
        for i in range(self.s_size):
            dim = self.s[i]
            indices = self.Pvector[c : c + dim]
            matrix = rot_map[dim]
            weight = self.w[i]
            self.sub_problems.append((indices, matrix, weight))
            c += dim
        self.c_end = c  # 记录主要部分结束的位置 / Record end of main part

    def __call__(self, x: Union[npt.NDArray, List]) -> npt.NDArray:
        """使函数可调用 / Make function callable."""
        return self.compute(x)

    def info(self) -> Dict[str, Union[int, float]]:
        """获取函数信息 / Get function information."""
        return {
            'best': 0.0,
            'dimension': self.dimension,
            'lower': self.minX,
            'upper': self.maxX
        }

    def compute(self, x: Union[npt.NDArray, List]) -> npt.NDArray:
        """
        计算 F4 函数值 / Compute F4 function value.

        实现逻辑 / Implementation logic:
            1. 全局位移 / Global shift
            2. 子问题循环 / Sub-problem loop
            3. 余项处理 / Remainder handling (if any)
        """
        if not isinstance(x, np.ndarray):
            x = np.array(x)
        if x.ndim == 1:
            x = x[np.newaxis, :]

        result = np.zeros(x.shape[0], dtype=x.dtype)

        # 全局位移 / Global shift
        z_global = x - self.Ovector

        # 子问题循环 / Sub-problems loop
        for indices, matrix, w in self.sub_problems:
            # 切片 -> 旋转 -> OSZ -> Elliptic / Slice -> Rotate -> OSZ -> Elliptic
            z_sub = z_global[:, indices] @ matrix.T
            z_sub = self.transform_osz(z_sub)
            result += w * self.elliptic(z_sub)

        # 余项处理（如果有） / Remainder handling (if any)
        if self.c_end < self.dimension:
            indices = self.Pvector[self.c_end : self.dimension]
            z_rem = z_global[:, indices]
            z_rem = self.transform_osz(z_rem)
            result += self.elliptic(z_rem)

        return result

"""
F6: 7-Subcomponent Rotated Ackley / 7 子组件旋转 Ackley 函数.

CEC 2013 大规模全局优化基准测试函数 F6。
CEC 2013 Large-Scale Global Optimization Benchmark Function F6.
...
class F6(Benchmarks):
    ...
    CEC 2013 基准测试函数 F6，使用 Ackley 函数作为子组件。
    CEC 2013 benchmark function F6 using Ackley function as sub-components.

    Attributes:
        ID: 函数标识符 / Function identifier (6)
    ...
    def __init__(self) -> None:
        """初始化 F6 函数 / Initialize F6 function."""
        super().__init__()
        self.ID = 6
        self.s_size = 7
    ...
    def compute(self, x: Union[npt.NDArray, List]) -> npt.NDArray:
        """
        计算 F6 函数值 / Compute F6 function value.

"""
F9: 20-Subcomponent Rotated Rastrigin / 20 子组件旋转 Rastrigin 函数.

CEC 2013 大规模全局优化基准测试函数 F9。
CEC 2013 Large-Scale Global Optimization Benchmark Function F9.
...
class F9(Benchmarks):
    ...
    CEC 2013 基准测试函数 F9，使用 Rastrigin 函数作为子组件。
    CEC 2013 benchmark function F9 using Rastrigin function as sub-components.

    Attributes:
        ID: 函数标识符 / Function identifier (9)
    ...
    def __init__(self) -> None:
        """初始化 F9 函数 / Initialize F9 function."""
        super().__init__()
        self.ID = 9
        self.s_size = 20
    ...
    def compute(self, x: Union[npt.NDArray, List]) -> npt.NDArray:
        """
        计算 F9 函数值 / Compute F9 function value.
